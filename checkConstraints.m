            ...
            switch eas.penalty_method
                case 'static'
                    for i=1:eas.n_individuals
                        ...
                        fit_array(i,eas.fitIdx.pen) = calculateStaticPenalty(pop(:,:,index),[100 100 100 100 100 100 100 100]);
                ...
                case 'adaptive'
                    for i=1:eas.n_individuals
                        index = fit_array(i,eas.fitIdx.id);
                        fit_array(i,eas.fitIdx.pen) = max(1, eas.adaptive_pen_mult) * calculateStaticPenalty(pop(:,:,index),[100 100 100 100 100 100 100 100]);
                        if eas.ranking_method == "penalty"
                            fit_array(i,eas.fitIdx.ik) = fit_array(i,eas.fitIdx.ik) + fit_array(i,eas.fitIdx.pen);
                        end
                    end
