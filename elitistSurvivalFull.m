...
function [nextGenPop_final, fit_array_NGP] = elitistSurvivalFull(pop, offspring, fit_array_P, fit_array_O)
    ...
    nextGenPop = zeros(size(op.targets,1)*2+1,op.n_links+eas.extra_genes,eas.n_individuals*2);    % declare a static array of chromosomes filled with zeros
    fit_array_NGP = zeros(eas.n_individuals*2,length(fieldnames(eas.fitIdx)));                  % array containing fitness for each individual of the new population
    
    nextGenPop(:,:,1:eas.n_individuals) = pop;
    nextGenPop(:,:,eas.n_individuals+1:eas.n_individuals*2) = offspring;
    
    fit_array_O(:,eas.fitIdx.id) = fit_array_O(:,eas.fitIdx.id) + eas.n_individuals;
    ...
    nextGenPop_final = zeros(size(op.targets,1)*2+1,op.n_links+eas.extra_genes,eas.n_individuals);
    for i=1:eas.n_individuals 
        index = fit_array_NGP(i,eas.fitIdx.id);
        nextGenPop_final(:,:,i) = nextGenPop(:,:,index);
        fit_array_NGP(i,eas.fitIdx.id) = i;
    end
end
